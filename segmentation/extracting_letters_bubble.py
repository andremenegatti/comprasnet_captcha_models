"""
This script iterates over 'bubble' or 'bubble_cut' CAPTCHAs, performs
character segmentation and stores individual character images in folders
whose names corresponds to the characters it contains.
...
Notes:
    ...
    substitute '/' for '\\' in the first cv2.imread command inside the loop.
"""
import os
import time
import numpy as np
...
INPUT_FOLDER = 'captchas/train/bubble_cut'
OUTPUT_FOLDER = 'letters/bubble_cut'
...
for index, captcha_image_file in enumerate(os.listdir(INPUT_FOLDER)):
    ...
    hcontours = cv2.findContours(thresh.copy(),
                                 cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    contours = hcontours[0]

    conts = [(c,) for c, x in enumerate(hcontours[1][0]) if x[3] == -1]

    for pingij_index, pingij in \
    [(index, cc) for index, cc \
    ...
    if len(conts) != 6:
        ...
        split_error.append((captcha_image_file, len(conts)))
        continue
...
cont_error_rate = round( len(cont_error) * 100/ total, ndigits=3)
split_error_rate = round( len(split_error) * 100/ total, ndigits=3)
point_error_rate = round( len(point_error) * 100/ total, ndigits=3)

print('cont error rate (perc):  ' + str(cont_error_rate))
print('point error rate (perc): ' + str(point_error_rate))
print('split error rate (perc): ' + str(split_error_rate))
print('TOTAL ERROR RATE (PERC): ' + \
      str(cont_error_rate + split_error_rate + point_error_rate))